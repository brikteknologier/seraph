...
var node = module.exports = {
  ...
  label: function(node, label, callback) {
    if (Array.isArray(node)) {
      var txn = this._safeBatch();
      var args = [key, val];
      async.map(obj, naan.ncurry(txn.save, label, 1), callback);
      return this._safeBatchCommit(txn);
    }
    ...
    var op = this.operation(endpoint, 'POST', label);
  ...
  nodesWithLabel: function(label, callback) {
    var endpoint = util.format('label/%s', label);
