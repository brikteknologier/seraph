...
var node = module.exports = {
   ...
   * cypher query.
   */
  find: function(predicate, any, start, label, callback) {
    ...
    if (Array.isArray(predicate)) {
      var txn = this._safeBatch();
      var finder = naan.ecurry(txn.node.find, [any, start, label], [1, 2, 3]);
    ...
    if (label) {
      matchers.push(util.format(" ('%s' IN LABELS(n)) ", label));
    }
