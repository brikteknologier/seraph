...
module.exports = {
  ...
  _update: function(obj, callback) {
    ...
    var endpoint = util.format('node/%s/properties', id);
  ...
  delete: function(id, force, callback) {
    ...
    function deleteRelations(cb) {
      function queryDone(err, data) {
        if (err)
          return cb(err);
        self.rel.delete(data, cb);
      }
      self.query("START n=node({id}) MATCH n-[r]-m RETURN r",
                 { id: id },
                 queryDone);
    }
    
    function deleteNode() {
      var endpoint = util.format('node/%s', id);
      var op = self.operation(endpoint, 'DELETE');
      self.call(op, function(err) {
        callback(err);
      });
    };
    
    if (force) {
      deleteRelations(function(err) {
        if (err)
          return callback(err);
        deleteNode();
      });
  ...
  relationships: function(obj, direction, relName, callback) {
    ...
    var endpoint = util.format('node/%s/relationships/%s', objId, direction);
    if (relName) {
      endpoint += "/" + relName;
    }
    var op = this.operation(endpoint);
