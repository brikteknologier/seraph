...
module.exports = {
  ...
  save: function(obj, callback) {
    if (Array.isArray(obj)) {
      return async.map(obj, this.node.save, callback);
    }
  ...
  _update: function(obj, callback) {
    ...
    var op = this.operation(endpoint, 'PUT', obj);
    this.call(op, function(err, body, response) {
      if (err) {
        return callback(err);
      } else {
  ...
  read: function(id, callback) {
    if (Array.isArray(id)) {
      return async.map(id, this.node.read, callback);
    }
  ...
  delete: function(id, callback) {
    if (Array.isArray(id)) {
      return async.map(id, this.node.delete, callback);
    }
