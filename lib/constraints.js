...
exports.uniqueness.createIfNone = function(label, keys, callback) {
  var self = this;
  exports.uniqueness.create.call(this, label, keys, function(err, constraint) {
    if (!err) callback(null, constraint);
    else if (err.statusCode != 409) callback(err);
    else {
      exports.uniqueness.list.call(self, label, keys, function(err, constraint) {
        if (err) callback(err);
        else callback(null, constraint[0]);
      });
    }
  });
};
