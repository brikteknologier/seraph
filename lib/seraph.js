...
function BatchSeraph(options) {
  ...
  function wrapFunction(fn) {
    ...
    return function() {
      ...
      switch (operationIds.length) {
        ...
        case  1: return operationIds[0];
        default: return operationIds;
      }
    };
  }

  this.super = new Seraph(options);
  ...
  this.super.call = function(operation, processor) {
    ...
    self.processors.push(processor.bind(self));
    self.operationQueueStack[0].push(operation);
  }
}

Seraph.prototype.batch = function(operations, callback) {
  var batchSeraph = new BatchSeraph(this.options);
  operations(batchSeraph);
  
  var op = this.operation('batch', 'POST', batchSeraph.operations);
  this.call(op, function(err, results) {
    if (err) {
      return callback(err);
    }
