...
var seraph = {
   ...
   *   { method: 'PUT'|'POST'|'GET'|'DELETE''testIndex1'
   *   , to    : path,
   *   , body  : object }
   *
    ...
    this.queryRaw(options, query, params, function(err, result) {
      ...
      var namedResults = result.data.map(function(row) {
        return result.columns.reduce(function(rowObj, columnName, columnIndex) {
          ...
          if (self._isNode(options, resultItem)) {
            resultItem = self._createNodeObject(options, resultItem);
          } else if (self._isRelationship(options, resultItem)) {
            resultItem = self._createRelationshipObject(options, resultItem);
          }
...
naan.ecrock(seraph.node.index, seraph.index, naan.ncurry, 'node', 1);
naan.ecrock(seraph.rel.index, seraph.index, naan.ncurry, 'relationship', 1);
