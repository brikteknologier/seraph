...
class Seraph {
  ...
  session() { 
    ...
    var sessSeraph = seraph(this.options);
  ...
  transaction(aggregateResults) {
    ...
    if (aggregateResults) {
      var ops = [];
      var txnSeraph = new Proxy(new Seraph(this.options), { get: (target, key) => {
      ...
      txnSeraph.commit = () => {
        return txn.commit().then(() => Promise.all(ops))
      }
    } else {
      var txnSeraph = new Seraph(this.options);
  ...
  label(node, labels, replace) {
    if (Array.isArray(node)) return this._aggregate(this.label, node, 0, arguments);
  ...
  removeLabel(node, labels) {
    if (Array.isArray(node)) return this._aggregate(this.removeLabel, node, 0, arguments);
    if (!Array.isArray(labels)) labels = [labels];
