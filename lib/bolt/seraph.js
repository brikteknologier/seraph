...
module.exports = function(opts) {
  var db;
  if (!opts || !opts.nodeify) db =  new Seraph(opts);
  else {
    db = new Proxy(new Seraph(opts), {
...
class Seraph {
  ...
  _aggregate(fn, arr, argnum, allargs) {
    var sess = this.opts.session ? this : this.session();
  ...
  save(node, label) {
    ...
    return this._getId(node)
      .then(id => {
        ...
        return this._run(`MATCH (node) WHERE id(node) = {id}
                         SET node = {props} ${label}
                         RETURN node`, { id, props });
      })
      .catch(() => {
        return this._run(`CREATE (node) 
                         SET node = {node} ${label}
                         RETURN node`, {node})
      })
      .then((result) => this._processResult(result, true))
